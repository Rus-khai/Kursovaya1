...
from src.utils import get_date_time, get_filtered_by_date_range, top_5_transaction

def test_get_date_time():
    assert  get_date_time("2021-12-31 16:00:00") == ["01.12.2021 16:00:00", "31.12.2021 16:00:00"]


@mock.patch('pd.read_excel(data)')
def test_get_filtered_by_date_range(get_data, get_period_time):
    mock.return_value = get_data
    result = [
            {
    ...
    assert get_filtered_by_date_range(get_data, get_period_time) == pd.DataFrame(result)
...
#
@mock.patch('pandas.DataFrame.to_dict')
def test_top_5_transaction(df_transaction):
    result = [
        {
